...
if (isset($_POST['signup'])){
  ...
  if(count($errors) === 0){
    ...
    $status = "notverified";
    $insert_query = mysqli_query($con,"INSERT INTO users (name, email,phone, password,shippingAddress,shippingState, shippingCity, shippingPinCode,
       donationCoupon, code, status, userStatus)
                    VALUES('$name', '$email', '$phone','$hashedpwd', '$shippingAddress', '$shippingState', '$shippingCity' ,'$shippingPinCode','$donationCoupon','$code',
                      '$status', 1)");
    if($insert_query){
